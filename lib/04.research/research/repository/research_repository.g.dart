...
class _ResearchRepository implements ResearchRepository {
  ...
  @override
  Future<SurveyParticipationResponse> participateSurvey({required id}) async {
    const _extra = <String, dynamic>{};
    final queryParameters = <String, dynamic>{};
    final _headers = <String, dynamic>{r'accessToken': 'true'};
    _headers.removeWhere((k, v) => v == null);
    final Map<String, dynamic>? _data = null;
    final _result = await _dio.fetch<Map<String, dynamic>>(
        _setStreamType<SurveyParticipationResponse>(Options(
      method: 'POST',
      headers: _headers,
      extra: _extra,
    )
            ...
            .copyWith(baseUrl: baseUrl ?? _dio.options.baseUrl)));
    final value = SurveyParticipationResponse.fromJson(_result.data!);
    return value;
  }
